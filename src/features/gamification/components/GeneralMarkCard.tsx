...
export const GeneralMarkCard: React.FC<GeneralMarkCardProps> = ({ mark, isUnlocked }) => {
  ...
  const getVisualStyles = (visual: GeneralMark['visual']) => {
    // LOCKED STATE
    if (!isUnlocked) {
      return {
    ...
    // --- 3. SPECIFIC ANIMATION OVERRIDES ---
    ...
    if (overrides[mark.id]) {
        selectedStyle = { ...selectedStyle, iconAnim: overrides[mark.id] };
    }

    return selectedStyle;
  };

  const style = getVisualStyles(mark.visual);
